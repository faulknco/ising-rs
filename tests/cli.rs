...
fn sweep_outputs_csv_header_and_data() {
    let output = cargo_bin("sweep")
        .args(["--n", "4", "--geometry", "cubic", "--samples", "10", "--warmup", "10",
               "--tmin", "3.0", "--tmax", "5.0", "--steps", "3", "--seed", "1"])
        .output()
    ...
    for line in &lines[1..] {
        ...
        assert_eq!(fields.len(), 5, "expected 5 columns, got {}: {line}", fields.len());
    ...
    assert!((t_first - 3.0).abs() < 0.01, "first T should be ~3.0, got {t_first}");
    assert!((t_last - 5.0).abs() < 0.01, "last T should be ~5.0, got {t_last}");
}

#[test]
fn sweep_wolff_flag_works() {
    let output = cargo_bin("sweep")
        .args(["--n", "4", "--geometry", "cubic", "--wolff",
               "--samples", "10", "--warmup", "10",
               "--tmin", "3.0", "--tmax", "5.0", "--steps", "2", "--seed", "1"])
        .output()
...
fn fss_writes_csv_files() {
    ...
    let output = cargo_bin("fss")
        .args(["--sizes", "4,6", "--geometry", "cubic", "--wolff",
               "--samples", "10", "--warmup", "10",
               "--tmin", "3.0", "--tmax", "5.0", "--steps", "3", "--seed", "1",
    ...
    assert!(output.status.success(), "fss exited with error: {}",
...
fn fss_raw_mode_writes_timeseries() {
    ...
    let output = cargo_bin("fss")
        .args(["--sizes", "4", "--raw", "--wolff",
               "--samples", "5", "--warmup", "5",
               "--tmin", "4.0", "--tmax", "5.0", "--steps", "2", "--seed", "1",
               "--outdir", tmp.to_str().unwrap()])
        .output()
        .expect("failed to run fss --raw");

    assert!(output.status.success(), "fss --raw exited with error: {}",
            String::from_utf8_lossy(&output.stderr));
    ...
    let content = std::fs::read_to_string(&path).unwrap();
    let lines: Vec<&str> = content.trim().lines().collect();
    assert_eq!(lines[0], "T,sample,e,m_abs,m_signed", "bad header in raw CSV");
    // 2 temperatures * 5 samples = 10 data rows
    assert_eq!(lines.len(), 11, "expected header + 10 rows, got {}", lines.len());
...
fn mesh_sweep_with_json_graph() {
    ...
    std::fs::write(&graph_path, r#"{
        "n_nodes": 4,
        "edges": [[0,1],[1,2],[2,3],[3,0]]
    }"#).unwrap();

    let output = cargo_bin("mesh_sweep")
        .args(["--graph", graph_path.to_str().unwrap(),
               "--j", "1.0", "--tmin", "1.0", "--tmax", "3.0", "--steps", "3",
               "--warmup", "10", "--samples", "10", "--seed", "1",
               "--outdir", tmp.to_str().unwrap(), "--prefix", "ring4"])
        .output()
        .expect("failed to run mesh_sweep");

    assert!(output.status.success(), "mesh_sweep exited with error: {}",
            String::from_utf8_lossy(&output.stderr));
...
fn kz_writes_csv() {
    ...
    let output = cargo_bin("kz")
        .args(["--n", "4", "--geometry", "cubic",
               "--tau-min", "10", "--tau-max", "100", "--tau-steps", "3",
               "--trials", "2", "--seed", "1",
               "--outdir", tmp.to_str().unwrap()])
        .output()
        .expect("failed to run kz");

    assert!(output.status.success(), "kz exited with error: {}",
            String::from_utf8_lossy(&output.stderr));
...
fn coarsening_writes_csv() {
    ...
    let output = cargo_bin("coarsening")
        .args(["--n", "4", "--geometry", "cubic",
               "--t-quench", "3.0", "--steps", "50", "--sample-every", "25",
               "--warmup", "10", "--seed", "1",
               "--outdir", tmp.to_str().unwrap()])
        .output()
        .expect("failed to run coarsening");

    assert!(output.status.success(), "coarsening exited with error: {}",
            String::from_utf8_lossy(&output.stderr));
