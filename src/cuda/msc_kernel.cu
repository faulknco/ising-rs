...
extern "C" __global__ void msc_batched_metropolis_kernel(
    ...
    // Decode (wx, y, z) from local word index
    int wx = local_tid % wpr;
    int y  = (local_tid / wpr) % N;
    int z  = local_tid / (wpr * N);
    int x_base = wx * 32;
    ...
    unsigned int my_word = spins_all[offset + local_tid];

    // Neighbor words (within this replica's buffer)
    int wx_right = (wx + 1) % wpr;
    int wx_left  = (wx + wpr - 1) % wpr;
    unsigned int word_xp_raw = spins_all[offset + z * N * wpr + y * wpr + wx_right];
    unsigned int word_xm_raw = spins_all[offset + z * N * wpr + y * wpr + wx_left];
    unsigned int nb_xp = (my_word >> 1) | ((word_xp_raw & 1u) << 31);
    unsigned int nb_xm = (my_word << 1) | ((word_xm_raw >> 31) & 1u);

    int yp = (y + 1) % N;
    int ym = (y + N - 1) % N;
    unsigned int nb_yp = spins_all[offset + z * N * wpr + yp * wpr + wx];
    unsigned int nb_ym = spins_all[offset + z * N * wpr + ym * wpr + wx];

    int zp = (z + 1) % N;
    int zm = (z + N - 1) % N;
    unsigned int nb_zp = spins_all[offset + zp * N * wpr + y * wpr + wx];
    unsigned int nb_zm = spins_all[offset + zm * N * wpr + y * wpr + wx];
    ...
    // Carry-save addition (same as single-replica kernel)
    unsigned int s1 = anti_xp ^ anti_xm ^ anti_yp;
    unsigned int c1 = (anti_xp & anti_xm) | (anti_xp & anti_yp) | (anti_xm & anti_yp);
    unsigned int s2 = anti_ym ^ anti_zp ^ anti_zm;
    unsigned int c2 = (anti_ym & anti_zp) | (anti_ym & anti_zm) | (anti_zp & anti_zm);
    ...
    flip_mask &= parity_mask;
    spins_all[offset + local_tid] = my_word ^ flip_mask;
    rng_states[tid] = local_rng;
}

// Batched MSC reduction: magnetisation for one replica.
// Sums words in range [offset .. offset + words_per_replica)
extern "C" __global__ void msc_batched_reduce_mag(
    const unsigned int* spins_all,
    float* partial_mag,
    int words_per_replica,
    int offset
) {
    extern __shared__ float sdata[];
    int tid = threadIdx.x;
    ...
    float val = 0.0f;
    if (gid < words_per_replica) {
        int up = __popc(spins_all[offset + gid]);
        val = (float)(2 * up - 32);
    }
    sdata[tid] = val;
    __syncthreads();
...
// Batched MSC reduction: energy for one replica.
extern "C" __global__ void msc_batched_reduce_energy(
    const unsigned int* spins_all,
    float* partial_energy,
    int N,
    float J,
    ...
    if (gid < words_per_replica) {
        ...
        unsigned int my_word = spins_all[offset + gid];

        int wx_right = (wx + 1) % wpr;
        unsigned int word_xp = spins_all[offset + z * N * wpr + y * wpr + wx_right];
        unsigned int nb_xp = (my_word >> 1) | ((word_xp & 1u) << 31);
